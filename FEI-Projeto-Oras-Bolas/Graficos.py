...
# ------------------------------Calculo da Média da acl do robo-----------------
for i in range(len(acl_robo)):
    if(T_robo[i] <= 1):
        acl_robo[i] = 2.8
        acl_robo_X[i] = 2.8
        acl_robo_Y[i] = 2.8
...
media = sum(acl_bola)/len(acl_bola)
for i in range(len(acl_bola)):
    acl_bola[i] = 0
media = sum(acl_bola_X)/len(acl_bola_X)
for i in range(len(acl_bola_X)):
    acl_bola_X[i] = 0
media = sum(acl_bola_Y)/len(acl_bola_Y)
for i in range(len(acl_bola_Y)):
    acl_bola_Y[i] = 0
media = sum(vel_bola)/len(vel_bola)
for i in range(len(vel_bola)):
    vel_bola[i] = media
media = sum(vel_bola_X)/len(vel_bola_X)
for i in range(len(vel_bola_X)):
    vel_bola_X[i] = media
media = sum(vel_bola_Y)/len(vel_bola_Y)
for i in range(len(vel_bola_Y)):
    vel_bola_Y[i] = media
